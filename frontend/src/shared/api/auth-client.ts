...
export const authClient = {
  register: async (payload: AuthRegisterRequest) => {
    const response = await httpClient.post<AuthTokenResponse>('/api/v1/auth/register', payload)
    return response.data
  },
  login: async (payload: AuthLoginRequest) => {
    const response = await httpClient.post<AuthTokenResponse>('/api/v1/auth/login', payload)
    return response.data
  },
  refresh: async (payload: AuthRefreshRequest) => {
    const response = await httpClient.post<AuthTokenResponse>('/api/v1/auth/refresh', payload)
    return response.data
  },
